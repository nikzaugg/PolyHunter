...
void ModelRenderer::drawModelInstanced(const std::string &modelName, const std::string &cameraName, const vmml::Matrix4f & modelMatrix, const std::vector<std::string> &lightNames, bool doFrustumCulling, bool cullIndividualGeometry)
{
	drawModelInstanced(_objectManager->getModel(modelName), modelMatrix, _objectManager->getCamera(cameraName)->getViewMatrix(), _objectManager->getCamera(cameraName)->getProjectionMatrix(), lightNames, doFrustumCulling, cullIndividualGeometry);
}

void ModelRenderer::drawModelInstanced(ModelPtr model, const vmml::Matrix4f &modelMatrix, const vmml::Matrix4f &viewMatrix, const vmml::Matrix4f &projectionMatrix, const std::vector<std::string> &lightNames, bool doFrustumCulling, bool cullIndividualGeometry)
{
	vmml::Matrix4f modelViewMatrix = viewMatrix * modelMatrix;
	vmml::Matrix4f modelViewProjectionMatrix = projectionMatrix * modelViewMatrix;
	vmml::Matrix3f normalMatrix;
